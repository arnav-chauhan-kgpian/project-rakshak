...
class QdrantUpsertAgent:
    ...
    def upsert_to_memory(self, embedding, metadata, incident_id, sparse_vector=None):
        ...
        try:
            ...
            if self.named_dense:
                vectors = {}
                # Add image embedding if mapped
                if "image" in self.dense_names:
                    vectors["image"] = embedding
                else:
                    # Fallback if image not mapped but collection is named (unlikely based on init)
                    vectors["default"] = embedding
                
                # Add audio embedding if provided and mapped
                if "audio" in self.dense_names and _safe_get(metadata, "audio_embedding"):
                    vectors["audio"] = metadata["audio_embedding"]
            else:
            ...
            # Add audio info to payload
            if metadata.get("audio_transcript"):
                payload["audio_transcript"] = metadata.get("audio_transcript")
                payload["audio_panic_score"] = metadata.get("audio_panic_score", 0.0)
